...
def site_processing(site): # обработка сайта
    ...
    try: # попытаться открыть сайт
        ...
        if err < 0: # если ошибок меньше задонного числа
            err += 1 # добавляем значение к счётчику ошибок
            time.sleep(5) # ждём n секунд

        else:
            Windows_msg(Name_app = "Уведомление с форума ASCON", title = "Отслеживание сообщений прекращено!", msg = "Последняя ошибка:" + print(e), duration = "short", icon = icon, launch = "") # сообщение Windows
            exit()
...
def ASCON(): # уведомление от ASCONa
    ...
    ##            WindowsBalloonTip(topic + "От " + autor + ":" + message) # всплывающие сообщения в Windows (исчезает по истечении времени)
            ...
            Windows_msg(Name_app = "Уведомление с форума ASCON", title = topic, msg = "От " + autor + ":" + message, duration = "short", icon = icon, launch = launch) # сообщение Windows
