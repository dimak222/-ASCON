...
def Windows_ver(ver): # проверка версии Windows ("7;8;8.1")
...
def error(e = "Ошибка!"): # в случае ошибки пытаеться 10 повторить выполнение программы (имя ошибки)
    ...
    if err <= 10: # если ошибок меньше задонного числа
        ...
        Windows_msg(Name_app = "Уведомление с форума ASCON", title = "Отслеживание сообщений прекращено!", msg = "Последняя ошибка: " + "\"" + str(e) + "\"", duration = "short", icon = icon, launch = "") # сообщение Windows (наименование уведомления, заголовок, сообщение, длительность отображения, полный путь к иконки, ссылка при нажатии уведомления)
        exit() # выходим из програмы
...
Windows_ver("7;8;8.1") # проверка версии Windows ("7;8;8.1")
