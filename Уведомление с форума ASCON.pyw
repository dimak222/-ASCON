...
while 1: # цикл обработки сообщений
    ...
    try: # попытаться выполнить программу
        ...
        if int(update_msg[0]) < 20: # если нижний порог меньше 20 сек присвоить ему 20 сек (что бы не DDosить)
            update_msg[0] = 20 # 20 сек нижний порог

        if int(update_msg[1]) < 40: # если верхний порог меньше 40 сек присвоить ему 40 сек (что бы не DDosить)
            update_msg[1] = 40 # 40 сек верхний порог

        Sleep(int(update_msg[0]), int(update_msg[1])) # остановка на случайное от n до m секунд
